...
ErrorMsg()
{
Message=( "out of synchronization" "not included" "file does not exist" "HUP signal")
for j in "${Message[@]}"
	do
...
for i in $ServerList
	do
	pgrep -af iqsrv16 | grep $i > /dev/null
	a=$?	
	dbping -q -c dsn=$i > /dev/null
	b=$?
	if [ "$a" != 0 ] && [ "$b" != 0 ];then
		echo -e "\e[31m$(TimeStamp) $i is not running\e[0m" >> $LOGIFLE
		echo -e "\e[32m$(TimeStamp) Trying to start server $i\e[0m" >> $LOGIFLE
		...
		if [ "$?" == 0 ];then
			...
			case $text in
				"out of synchronization")
					...
					;;
				"HUP signal")
					echo -e "\e[31m$(TimeStamp) $i is down due to HUP signal\e[0m" >> $LOGIFLE
                    echo -e "\e[31m$(TimeStamp) Resyncing $i Mutliplex Server\e[0m" >> $LOGIFLE
                    /opt/sybase/scripts/resync.sh --from $COORD --to $i >> $logfile
                    if [ "$?" == 0 ];then echo -e "\e[32m$(TimeStamp) $i started successfully\e[0m" >> $LOGIFLE; fi
                    ;;
